...
void setup() {
    ...
    attachInterrupt(digitalPinToInterrupt(DISPENSE_CALIBRATE_PIN), setPositionZeroISR, FALLING);
    attachInterrupt(digitalPinToInterrupt(DISPENSER_CONTROLLER_PIN), controllerISR, CHANGE);
    dispenser.setSpeed(15);
    Serial.begin(9600);
}

void loop() {
    // Check if the dispenser is in use already.
    if (dispensing) {
